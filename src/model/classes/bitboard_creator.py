...
class Bitboard_Creator:
    ...
    def get_training_features(self,board: chess.Board):
        self.setup_parameters_board(board = board)
        dict_results = {}

        dict_results["positions_data"] = get_bitboards(self.board)
        dict_results["game_results"] = list(self.get_game_results().values())

        return dict_results    

def get_bitboards(board):
    bitboards = piece_bitboards(board)
    bitboards.append(en_passant_bitboard(board=board))

  
def piece_bitboards(board):
    bitboards = []
